...
import os
import sys
from cases import isIp_in_file, isConnect, isLNodesEqual
...
class IECTestCase(unittest.TestCase):
    ...
    def test_IP_is_in_file(self):
        print('\ntest IP is in file?')
        self.assertTrue(isIp_in_file(filename = self.FILENAME, ied_ip=self.IP))
        print('Success')

    def test_Connect(self):
        print('\ntest Connection to IED')
        self.assertTrue(isConnect(ied_ip=self.IP))
        print('Success')


    def test_Validate_LNodes(self):
        print('\nvalidate Logical Nodes in IED')
        self.assertTrue(isLNodesEqual(filename = self.FILENAME, ied_ip=self.IP))
        print('Success')
